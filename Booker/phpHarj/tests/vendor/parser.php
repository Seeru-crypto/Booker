...
class HtmlValidator {
    ...
    public function validate($htmlSource) {
        try {
            $tokens = (new HtmlLexer($htmlSource))->tokenize();
            ...
            return $this->error($e, $htmlSource);
        }

        return null;
    }

    private function error($e, $htmlSource) {
        $message = sprintf("%s \nin program output %s (line:column)\n",
            $e->message,
            $this->locationString($e->pos, $htmlSource));

        return $message;
    }

    private function locationString($pos, $htmlSource) {
        $textParsed = substr($htmlSource, 0, $pos);
        $lines = explode("\n", $textParsed);
        $lineNr = count($lines);
...
function parseHtml($htmlSource) {

//    try {
        //(new tplLib\SourceParser($htmlSource))->parse();

        $tokens = (new tplLib\HtmlLexer($htmlSource))->tokenize();

        (new tplLib\HtmlParser($tokens))->parse();
